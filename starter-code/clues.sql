...
SELECT name, code, region, population FROM country WHERE region = 'Southern Europe' ORDER BY population ASC LIMIT 1;
...
SELECT region, SUM(population) AS sum_population FROM country GROUP BY region;

SELECT countrycode, COUNT(countrycode) AS count_language FROM countrylanguage GROUP BY countrycode ORDER BY count_language DESC;
